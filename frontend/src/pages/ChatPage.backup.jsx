          ...
          <div key={m.localId || m.id} className={`msg ${m.me ? 'me' : 'them'}`}>
            {m.media && (
              <div className="msg-media">
                {m.media.type && m.media.type.startsWith('image') && <img src={(import.meta.env.VITE_API_URL || 'http://localhost:8000') + m.media.url} alt={m.media.name} style={{ maxWidth: '240px' }} />}
                {m.media.type && m.media.type.startsWith('video') && <video src={(import.meta.env.VITE_API_URL || 'http://localhost:8000') + m.media.url} controls style={{ maxWidth: '320px' }} />}
                {!m.media.type && <a href={(import.meta.env.VITE_API_URL || 'http://localhost:8000') + m.media.url} target="_blank" rel="noreferrer">{m.media.name}</a>}
              </div>
            )}
